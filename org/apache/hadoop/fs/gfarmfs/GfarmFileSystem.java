...
public class GfarmFileSystem extends FileSystem {
    ...
    public boolean rename(Path src, Path dst) throws IOException {
        ...
        int err = GfarmFSNative.rename(srcPath, destPath);
        if (err != 0)
            throw new IOException(GfarmFSNative.getErrorString(err));
        return true;
    }
    ...
    public long getFileSize(Path path) {
        ...
        return GfarmFSNative.getFileSize(srep);
    }
    ...
    public boolean delete(Path path, boolean recursive) throws IOException {
        ...
        if (!exists(path))
            return false;
        if (GfarmFSNative.isFile(srep)) {
            err = GfarmFSNative.remove(srep);
            if (err != 0)
                throw new IOException(GfarmFSNative.getErrorString(err));
            return true;
        }
    ...
    public FileStatus[] listStatus(Path path) throws IOException {
        ...
        String[] entries = null;
        try {
            entries = GfarmFSNative.readdir(srep);
        } catch (Exception e) {
            return null;
        }
    ...
    public boolean mkdirs(Path path, FsPermission permission)
        ...
        if (dirParts.length > 0) {
            ...
            for (String dirPart : dirParts) {
                ...
                int err = GfarmFSNative.mkdir(dirName);
                if (err != 0)
                    throw new IOException(GfarmFSNative.getErrorString(err));
                dirName += "/";
            }
    ...
    public FileStatus getFileStatus(Path path) throws IOException {
        Path absolute = makeAbsolute(path);
        String srep = absolute.toUri().getPath();
        if (GfarmFSNative.isDirectory(srep)) {
            return new FileStatus(0, true, 1, 0, GfarmFSNative.getModificationTime(srep),
                    path.makeQualified(this));
        } else {
            return new FileStatus(GfarmFSNative.getFileSize(srep),
                    false,
                    (int) GfarmFSNative.getReplication(srep),
                    getDefaultBlockSize(path),
                    GfarmFSNative.getModificationTime(srep),
                    path.makeQualified(this));
        }
    }
    ...
    public BlockLocation[] getFileBlockLocations(FileStatus file, long start,
        ...
        String[] hints = GfarmFSNative.getDataLocation(pathString, start, len);
